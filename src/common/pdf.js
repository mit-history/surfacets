  ...
  saveRecord(record, pack, name) {
    ...
    record.pieces.forEach(piece => {
      Object.keys(piece).forEach(key => {
        this.addTextLine(I18N.get('record-' + key, pack), piece[key], this.y);
        this.y += 0.5;
      });
      this.y -= 0.3;
    ...
    this.addTextLine(I18N.get('record-receipts', pack), I18N.format('record-pounds', pack, record.receipts), this.y);
    this.y += 0.3;
    this.checkPage();
